...
const imageArray = images.map(([id, mobilePic, desktopPic, title]) => (
	<picture className='max-md:w-[32.4rem] relative' key={id}>
		<source media='(min-width: 768px)' srcSet={desktopPic} />
		<img className='brightness-[0.6]' src={mobilePic} alt='' />
		<p className='font-josefin absolute bottom-[1rem] left-[1.9rem] w-[9ch] text-[1.8rem] font-[300] uppercase text-white opacity-[0.75]'>
			{title}
		</p>
	</picture>
));
...
function Gallery() {
	return (
		<section className='grid-in-stylesheet mt-[7rem] flex flex-col items-center gap-[2rem]'>
			...
			{imageArray}
			<button className='btn hover:text-White max-md:mt-[1rem] hover:bg-gray-600'>See All</button>
		</section>
	);
}
